...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "puzcoin" is a composite category enabling all Puzcoin-related debug output
            if(ptrCategory->count(string("puzcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\puzcoincore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\puzcoincore
    // Mac: ~/Library/Application Support/puzcoincore
    // Unix: ~/.puzcoincore
#ifdef WIN32
    // Windows
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty puzcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override puzcoin.conf
