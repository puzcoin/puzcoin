...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-alias         - Vote on a governance object by masternode alias (using "MASTERNODE_CONF_FILENAME" setup)\n"
                "  vote-conf          - Vote on a governance object by masternode configured in " COIN_CONF_FILENAME "\n"
                "  vote-many          - Vote on a governance object by all masternodes (using "MASTERNODE_CONF_FILENAME" setup)\n"
                );
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair(COIN_CONF_FILENAME, statusObj));
