...
{
    if(fLiteMode) return; // ignore all Puzcoin related functionality
...
/*
    ...
    allow endless transaction that would bloat Puzcoin and make it unusable. To
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if(out.nValue == MASTERNODE_COIN_NEEDED*COIN && out.scriptPubKey == payee) return true;
    }
...
{
    if(fLiteMode) return; // disable all Puzcoin specific functionality
    ...
    RenameThread("puzcoin-privatesend");
